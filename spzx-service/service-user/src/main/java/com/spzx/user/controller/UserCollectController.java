...
public class UserCollectController {
    ...
    public Result<UserInfoVo> select(@PathVariable(name = "page") Integer page,
        ...
        PageInfo<UserCollect> pageInfo = userCollectService.select(page, limit);
        return Result.build(pageInfo, ResultCodeEnum.SUCCESS);
    }

    @Operation(summary = "商品是否收藏查询")
    @GetMapping(value = "/auth/select/collect/skuId")
    public Result<Boolean> selectBySkuId(@PathVariable(value = "productId") Long productId) {
        Boolean isCollect = userCollectService.selectBySkuId(productId);
